...
- **인포그래픽**: 
    ...
    - **생년별/성별 분포**: 연도별 인원 구성(Bubble Chart)과 성별 밸런스(Balance Scale)를 시각적으로 제공합니다.
...
- **필터링**: 생년, 지역, 역할별로 다중 선택 필터를 적용하여 원하는 그룹만 빠르게 찾습니다.
...
- **v4.4**: 생년별 참가 현황 차트 내 '실인원(중복 제거)' 계산 로직 적용
- **v4.3**: '참가 횟수' -> '참가 인원수', '주간 브리핑' -> '월간 브리핑' 레이블 변경
- **v4.2**: '생년별 참가 현황' 차트 프리미엄 디자인 적용 및 전체 기수 표시 로직 개선
- **v4.1**: AI 브리핑 버튼 사이드바 이동, 명예의 전당 내 '생년별 참가 현황' 차트 추가
- **v4.0**: 대시보드 인포그래픽 PDF 출력 모드 및 브라우저 인쇄 최적화 추가
- **v3.9.4**: 필터링용 임시 컬럼('month')이 DB 저장 시 포함되는 버그 수정
- **v3.9.3**: 공지 관리 ID 수동 입력 활성화 및 자동 추출 로직 수정
- **v3.9.2**: 공지 관리 개편 (산행 일정 -> 공지 관리), 이벤트 ID 자동 추출 로직 추가
